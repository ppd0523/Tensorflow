...
CHECK_POINT_DIR = TB_SUMMARY_DIR = "./tb/emg"
...
xy = np.loadtxt('./emg/2n_angle_zeroADC_sd.txt', delimiter=' ')
...
X = tf.placeholder(tf.float32, shape=[None, seq_length, inputDim])
...
with tf.variable_scope('Fully_connected_layer1'):
    W1_FC= tf.Variable(tf.random_normal([hiddenDim1, hiddenDim2]), dtype=tf.float32 ,name="w_fc")
    B1_FC= tf.Variable(tf.random_normal([hiddenDim2]), dtype=tf.float32 , name="b_fc")
    H1_FC = tf.nn.relu( tf.matmul(X_FC, W1_FC)+ B1_FC )
...
with tf.variable_scope('Fully_connected_layer2'):
    W2_FC= tf.Variable(tf.random_normal([hiddenDim2, outputDim]), dtype=tf.float32 ,name="w_fc")
    B2_FC= tf.Variable(tf.random_normal([outputDim]), dtype=tf.float32 , name="b_fc")
